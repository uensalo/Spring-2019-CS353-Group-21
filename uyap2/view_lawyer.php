...
<body>
	...
	if ($_SESSION['user_type'] == 'Lawyer') {
		if (isset($_POST['Subbmit'])) {
			...
			window.location='view_lawyer.php';
			</script>";
			}
		}
	...
	<table class="w3-table-all w3-centered w3-hoverable">
	  <tr>
		...
		$sql = "SELECT Case_ID, MAX(T_Date), Open_Date FROM Involved NATURAL JOIN TakesPlaceOn NATURAL JOIN Court_Case WHERE Lawyer_ID = '$tc1' AND Case_State = 'On-Going' GROUP BY Case_ID";
		$result = mysqli_query($db, $sql) or die('Error updating database: ' . mysql_connect_error($result));
		while ($table = mysqli_fetch_array($result)) {
			echo "<tr><form action='view_lawyer.php' method='post'>";
			echo ("<th>" . $table[0] . "</th>");
			echo ("<th>" . $table[2] . "</th>");  //Open date
			$sql1 = "SELECT FullName FROM Involved JOIN Citizen ON (Involved.Litigant_ID = Citizen.TC_ID) WHERE Case_ID = '$table[0]' AND Lawyer_ID = '$tc1' ";
			//echo($name . " &nbsp;&nbsp;  ");
	...
	<table class="w3-table-all w3-centered w3-hoverable">
	  <tr>
		...
		$sql = "SELECT Case_ID,MAX(T_Date), Open_Date FROM Involved NATURAL JOIN TakesPlaceOn NATURAL JOIN Court_Case WHERE Lawyer_ID = '$tc1'AND Case_State = 'Closed' GROUP BY Case_ID";
		$result = mysqli_query($db, $sql) or die('Error updating database: ' . mysql_connect_error($result));
		while ($table = mysqli_fetch_array($result)) {
			echo "<tr><form action='view_lawyer.php' method='post'>";
			echo ("<th>" . $table[0] . "</th>");
