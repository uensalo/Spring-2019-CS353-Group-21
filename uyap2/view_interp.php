...
<head>
	...
	<title>UYAP2 - Interpreter View</title>
</head>
...
<body>
	...
	$lang = "SELECT Language_Name FROM Translates WHERE TC_ID = '$tc'";
	$langResult = mysqli_query($db, $lang) or die('Error updating database: ' . mysql_connect_error($result)); 

	while ($table = mysqli_fetch_array($result)) {
		$name = $table[0];
		$lang = mysqli_fetch_array($langResult);
		echo ("<div class=\"w3-top\"><div class=\"w3-bar w3-border w3-indigo w3-text-white\"> <a href=\"view_interp.php\" class=\"w3-bar-item w3-button w3-padding-16\">
		<i class=\"material-icons\">person</i></a><div class=\"w3-bar-item w3-padding-16\">" . $name . "&nbsp" . $tc .  "&nbsp" . $lang[0] ."</div><div class=\"w3-bar-item w3-right\"><form action='view_interp.php' method='post'><input type='submit' class=\"w3-input w3-indigo w3-hover-grey\"type=\"submit\" style=\"margin: 0 auto;\" name='Log' value='Logout' style='width:65%; height:65%'></form></div><div class=\"w3-bar-item w3-right w3-padding-16\" style=\"display:inline-block; margin=1%;\"><strong>Interpreter</strong></div></div></div>");
	}
	if ($_SESSION['user_type'] == 'Interpreter') {
		$cases;
		$i = 0;
	...
	<table class="w3-table-all w3-centered w3-hoverable">
	  <tr>
		...
		$date = date('Y-m-d');
		$sql = "SELECT DISTINCT Case_ID,MAX(T_Date),Open_Date FROM Translates NATURAL JOIN TakesPlaceOn NATURAL JOIN Court_Case WHERE TC_ID= '$tc' AND Case_State = 'On-Going' GROUP BY Case_ID";
		$result = mysqli_query($db, $sql) or die('Error updating database: ' . mysql_connect_error($result));
		while ($table = mysqli_fetch_array($result)) {
			echo "<tr><form action='view_interp.php' method='post'>";
			echo ("<th>" . $table[0] . "</th>");
