...
<body>
	...
	<table class="w3-table-all w3-centered w3-hoverable">
	  <tr>
		...
		$date = date('Y-m-d');
		//echo $date;
		$sql = "SELECT Case_ID,MAX(T_Date), Open_Date FROM Judges NATURAL JOIN TakesPlaceOn NATURAL JOIN Court_Case WHERE TC_ID = '$tc1' AND Case_State = 'On-Going' GROUP BY Case_ID";
		$result = mysqli_query($db, $sql) or die('Error updating database: ' . mysql_connect_error($result));
		while ($table = mysqli_fetch_array($result)) {
			echo "<tr><form action='view_judge.php' method='post'>";
			echo ("<th>" . $table[0] . "</th>");
			//echo($tc . " &nbsp;&nbsp;  ");
	...
	<table class="w3-table-all w3-centered w3-hoverable">
	  <tr>
		...
		$sql = "SELECT Case_ID,MAX(T_Date), Open_Date FROM Judges NATURAL JOIN TakesPlaceOn NATURAL JOIN Court_Case WHERE TC_ID = '$tc1'AND Case_State = 'Closed' GROUP BY Case_ID";
		$result = mysqli_query($db, $sql) or die('Error updating database: ' . mysql_connect_error($result));
		while ($table = mysqli_fetch_array($result)) {
			echo "<tr><form action='view_judge.php' method='post'>";
			echo ("<th>" . $table[0] . "</th>");
		...
		for ($i = 0; $i < $ct; $i++) {
			...
			window.location='caseview_judge.php';
			</script>";
			}
		}
