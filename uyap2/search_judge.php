...
<head>
	...
	<title>UYAP2 - Judge Search View</title>
</head>
...
<body>
	...
	include("config.php");
	$tc = 	$_SESSION['tc'];
	$sql = "SELECT FullName,Gavel_Name FROM Judge NATURAL JOIN Citizen WHERE TC_ID = '$tc'";
	...
	<table class="w3-table-all w3-centered w3-hoverable">
	  <tr>
		...
		$result = mysqli_query($db, $sql) or die('Error updating database: ' . mysql_connect_error($result));
		while ($table = mysqli_fetch_array($result)) {
			echo "<tr><form action='search_judge.php' method='post'>";
			echo ("<th>" . $table[0] . "</th>");
			//echo($tc . " &nbsp;&nbsp;  ");
	...
	<table class="w3-table-all w3-centered w3-hoverable">
	  <tr>
		...
		if (isset($_POST['Subbmit'])) {
			...
			window.location='search_judge.php';
			exit;
			</script>";
			}
		}
